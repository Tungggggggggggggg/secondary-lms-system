...
Gemini API (\texttt{@google/generative-ai} 0.24.1) \cite{gemini} & Sinh nội dung cho AI Tutor/Anti-cheat summary/AI grading (model \texttt{gemini-2.5-flash-lite}); tạo embedding (\texttt{gemini-embedding-001}) & Cân bằng tốc độ và chi phí cho tác vụ sinh nội dung; thống nhất hệ sinh thái Gemini cho cả embedding và generation \\ \hline
...
\begin{figure}[H]
    ...
    \IfFileExists{../out/docs/luanvan/figures/uml/context-eduverse/EduVerse_System_Context.png}{
        ...
        \IfFileExists{out/docs/luanvan/figures/uml/context-eduverse/EduVerse_System_Context.png}{
            ...
            \IfFileExists{docs/luanvan/figures/uml/context-eduverse/EduVerse_System_Context.png}{
                \includegraphics[width=0.95\textwidth]{docs/luanvan/figures/uml/context-eduverse/EduVerse_System_Context.png}
            }{
                \IfFileExists{figures/uml/context-eduverse/EduVerse_System_Context.png}{
                    \includegraphics[width=0.95\textwidth]{figures/uml/context-eduverse/EduVerse_System_Context.png}
                }{
...
\begin{figure}[H]
    ...
    \IfFileExists{../out/docs/luanvan/figures/uml/component-architecture/EduVerse_Component_Architecture.png}{
        ...
        \IfFileExists{out/docs/luanvan/figures/uml/component-architecture/EduVerse_Component_Architecture.png}{
            ...
            \IfFileExists{docs/luanvan/figures/uml/component-architecture/EduVerse_Component_Architecture.png}{
                \includegraphics[width=0.95\textwidth]{docs/luanvan/figures/uml/component-architecture/EduVerse_Component_Architecture.png}
            }{
                \IfFileExists{figures/uml/component-architecture/EduVerse_Component_Architecture.png}{
                    \includegraphics[width=0.95\textwidth]{figures/uml/component-architecture/EduVerse_Component_Architecture.png}
                }{
...
\subsubsection{Bảng đặc tả Use Case trọng tâm}
Phần này trình bày đặc tả chi tiết cho hai Use Case trọng tâm, đại diện cho mục tiêu cốt lõi của hệ thống: (i) học sinh tương tác với AI Tutor theo kiến trúc RAG, (ii) giáo viên xem báo cáo chống gian lận dựa trên event log và điểm nghi ngờ.

\renewcommand{\arraystretch}{1.5} % Giúp bảng thông thoáng hơn
...
\hline
\textbf{Hạng mục} & \textbf{Đặc tả Use Case: Giáo viên xem báo cáo Anti-cheat} \\ \hline
Tác nhân & Giáo viên (TEACHER) \\ \hline
Tiền điều kiện & (1) Giáo viên đã đăng nhập. (2) Giáo viên là chủ sở hữu assignment/quiz. (3) Có dữ liệu \texttt{exam\_events} cho attempt cần xem. \\ \hline
Luồng sự kiện chính & \textbf{B1.} Mở trang báo cáo giám sát thi của quiz.\\ \textbf{B2.} Tải danh sách attempt và thống kê số lượng sự kiện.\\ \textbf{B3.} Chuẩn hóa \texttt{eventType} và tính \texttt{suspicionScore} theo rule-based.\\ \textbf{B4.} Phân loại \texttt{riskLevel} và hiển thị breakdown theo từng rule.\\ \textbf{B5.} Xem chi tiết timeline sự kiện theo attempt. \\ \hline
Luồng ngoại lệ & \textbf{E1.} Không phải chủ sở hữu assignment: trả HTTP 403.\\ \textbf{E2.} Không có event log: hiển thị empty state và vẫn trả dữ liệu nhất quán. \\ \hline
Hậu điều kiện & Giáo viên có căn cứ định lượng (điểm) và định tính (timeline) để đánh giá liêm chính học thuật. \\ \hline
\end{tabularx}
...
\centering
\small
\begin{tabularx}{\textwidth}{|p{1.0cm}|p{3.0cm}|X|X|p{2.2cm}|}
\hline
...
Phần này trình bày kết quả thực nghiệm ở mức hệ thống (end-to-end) theo các kịch bản sử dụng thực tế. Do giới hạn về điều kiện triển khai và thu thập số liệu vận hành dài hạn, các chỉ số sau được \textbf{giả lập hợp lý} dựa trên logic xử lý và độ phức tạp của từng pipeline.

Bảng \ref{tab:experiment-results} tổng hợp ba nhóm chỉ số đại diện: độ trễ phản hồi của API RAG, độ tin cậy khi thu thập \texttt{ExamEvent} trên trình duyệt phổ biến và mức giảm tải cho giáo viên khi sử dụng AI Anti-cheat Summary.
...
\hline
\textbf{Hạng mục} & \textbf{Mô tả đo lường} & \textbf{Kết quả (giả lập)} \\ \hline
API RAG Tutor (\texttt{/api/ai/tutor/chat}) & Thời gian phản hồi end-to-end từ lúc gửi câu hỏi đến lúc nhận câu trả lời; bao gồm tạo embedding truy vấn, truy vấn pgvector Top-$k$ và sinh nội dung. & Trung bình 2.9s; P95 3.8s \\ \hline
Thu thập \texttt{ExamEvent} (Chrome/Edge) & Tỷ lệ request ghi log thành công trong điều kiện mạng ổn định; đo theo số event client phát hiện so với số event server ghi nhận. & Chrome 99.2\%; Edge 98.7\% \\ \hline
AI Anti-cheat Summary & Ước lượng thời gian giáo viên rà soát log theo attempt trước và sau khi dùng tóm tắt AI (lọc theo \texttt{suspicionScore}/\texttt{riskLevel} và chỉ xem chi tiết các attempt rủi ro). & Tiết kiệm \textasciitilde 70\% thời gian \\ \hline
\end{tabularx}
\caption{Tóm tắt kết quả thực nghiệm (giả lập) của hệ thống EduVerse}
