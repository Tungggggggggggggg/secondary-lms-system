...
import { Mail, MessageSquare, Users, BookOpen, User } from "lucide-react";
...
export default function ParentTeachersPage() {
  ...
  const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { label: "Liên hệ Giáo viên", href: "/dashboard/parent/teachers" },
  ];
  ...
  return (
    <>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">Danh sách giáo viên từ các lớp học mà con bạn đã tham gia</p>
      </div>
      ...
      {!isLoading && !error && teachers.length === 0 && (
        <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
          ...
          <p className="text-gray-500 mb-6">
            Con bạn cần tham gia lớp học trước để có thể liên hệ với giáo viên
          </p>
        </div>
      )}
              ...
              <div className="mb-4 space-y-3">
                ...
                <div className="space-y-2">
                  {teacher.classrooms.map((classroom) => (
                    <div key={classroom.id} className="bg-gray-50 rounded-lg p-3">
                      <div className="text-sm font-medium text-gray-900 mb-1">
                        {classroom.name}
                      </div>
                      <div className="text-xs text-gray-500 mb-2">Mã: {classroom.code}</div>
                      {classroom.students && classroom.students.length > 0 && (
                        <div className="flex items-center gap-1 text-xs text-gray-600">
                          <User className="w-3 h-3" />
                          <span>
                            {classroom.students.map((s) => s.fullname).join(", ")}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div key={classroom.id} className="bg-gray-50 rounded-lg p-3">
                      ...
                      {classroom.students && classroom.students.length > 0 && (
                        <div className="text-xs text-gray-600 mt-1">
                          Con: {classroom.students.map((s) => s.fullname).join(", ")}
                        </div>
                      )}
                    </div>
                  ))}
