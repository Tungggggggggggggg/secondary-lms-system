...
export const GET = withRequestLogging(async (req: NextRequest) => {
  ...
  try {
    ...
    enforceRateLimit({ route: "admin.reports.growth", ip: (req.headers.get("x-forwarded-for") || req.ip || "").split(",")[0].trim(), userId: actor.id, limit: 60 });
    ...
    const data = await reportsRepo.growth(q.orgId || null);
