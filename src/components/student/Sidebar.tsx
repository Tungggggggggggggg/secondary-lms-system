    ...
    return (
        <aside
            className={`fixed left-0 top-0 h-full ${
                expanded ? "w-72" : "w-20"
            } bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 text-white shadow-2xl rounded-r-2xl z-50 transition-[width] duration-300 ease-in-out flex flex-col overflow-hidden`}
        >
            <div className={`${expanded ? "p-6" : "px-2 py-4"} flex h-full flex-col overflow-hidden`}>
                ...
                {/* Navigation */}
                <nav id="student-sidebar" className={`space-y-2 flex-1 overflow-y-auto ${expanded ? "pr-1" : "pr-0 [scrollbar-width:none] [&::-webkit-scrollbar]:w-0 [&::-webkit-scrollbar]:h-0"}`}>
                    {menu.map((item) => {
