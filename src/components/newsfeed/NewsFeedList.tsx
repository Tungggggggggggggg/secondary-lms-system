...
export default function NewsFeedList({ classroomId }: NewsFeedListProps) {
    ...
    const fetchPage = useCallback(
        async (targetPage: number) => {
            try {
                ...
                console.error(
                    `[ERROR] fetchPage for classroom ${classroomId}:`,
                    e
                );
            }
        },
    ...
    // Map recent comments từ hook sang CommentItem format (giữ nguyên nested structure với replies)
                ...
                <div className="space-y-4">
                    {posts.map((p) => (
                        <PostCard
                            ...
                            recentCommentsLoading={
                                recentCommentsLoading[p.id] || false
                            }
